...
def _build(database: Any, start_date: str, end_date: str, report_type: str) -> bytes:
    ...
    _write_table(workbook, "11_共享_计算口径", ["模块", "指标", "口径"], [
        ...
        ["采购", "预测日销量", "近7日日均销量×40% + 近30日日均销量×60%；用于库存天数、预计缺货日和下单时机，不再叠加趋势倍率。"],
        ["采购", "计算采购量", "A为近7日销量总量；按生产周期采用4A、7A或8A目标库存，再乘趋势倍率，采购量直接使用该目标库存，不扣减可用库存、采购在途或建议调拨；达到最低起订量后按50件倍数取整，余数1-10向下、11及以上向上。"],
        ["采购", "起订量处理", "最终建议量取计算采购量与最低起订量的较大值；差额单独提示运营确认。"],
        ["采购", "采购计划", "五个运营仓全部启用 SKU；宏博/铠博基础款固定下单日为每月5日、20日，紧急缺货时忽略固定日。含在途库存天数严格小于完整交付周期才标红；一周内黄色、一周外蓝色、清仓候选紫色。"],
    ])
    output = BytesIO()
